...
```{r}
left_join(trips_new2, Trips_date) %>% 
  group_by(client, day = wday(date, label = TRUE)) %>% 
  summarize(num_by_day = n()) %>% 
  pivot_wider(names_from = client, values_from = num_by_day) %>% 
  replace(is.na(.), 0) %>% 
  mutate(Casual = Casual/sum(Casual), Registered = Registered/sum(Registered))
  
```

We can see that casual clients were more likely to rent bikes on Saturdays (proportion of 0.56), of the departures in our largest departures dataset. Additionally, registered clients were more likely to rent bikes on Thursdays (proportion of 0.34). Some assumptions could be made to explain this discrepancy, such as casual bikers renting on weekends as an activity, while registered bikers incorporate renting bikes into their general routine/lifestyle. Additionally, there is no row for Fridays, most likely because the most departures never occurred on a Friday.
...
kids_lib <- kids %>% 
  ...
  filter(variable == "lib") %>% 
  group_by(state, year) %>% 
  summarize(lib_perchild = sum(inf_adj_perchild)) %>% 
  filter(year %in% c(1997, 2016)) %>% 
  mutate(thous_perchild = round(lib_perchild*1000, digits = 0)) %>% 
  summarize(state, year, thous_perchild, diff_perchild = diff(thous_perchild)) %>% 
  filter(year == 1997) %>% 
  mutate(add_diff = thous_perchild + diff_perchild)

ggplot(kids_lib, aes(x = year, y = thous_perchild)) +
  geom_segment(aes(xend = year + 19, yend = thous_perchild + diff_perchild), 
  ...
  geom_text(label = kids_lib$thous_perchild, size = 2, hjust = 0.1, vjust = 0.1) +
  geom_text(label = kids_lib$add_diff, size = 2, hjust = -2.5, vjust = -1)
```
